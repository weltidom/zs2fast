...
while i < n:
    ...
    if ROOT in path and '/ParameterListe/' not in path and 'ParameterListe' not in path:
        if dtype in [0xAA, 0x00]:
            ...
            if i+need<=n and cnt>0 and bpi>0:
                if sub == 0x4: preview = struct.unpack('<f', data[i:i+4])[0]
                elif sub == 0x5: preview = struct.unpack('<d', data[i:i+8])[0]
                elif sub == 0x16: preview = struct.unpack('<I', data[i:i+4])[0]
                elif sub == 0x11: preview = data[i]
            rows.append((path, f'EE{sub:04X}[{cnt}]', preview))
            i += need
            continue
        elif dtype == 0xDD:
            i += 1
            ...
            rows.append((path, 'section', f'dd_len={dd_len}'))
            continue
            ...
            if len(raw4)==4:
                ...
                elif dtype==0x44: val = struct.unpack('<f', raw4)[0]
            rows.append((path, {0x11:'u8',0x22:'i32',0x33:'u32',0x44:'f32'}[dtype], val))
            i += 4
            continue
        elif dtype == 0xCC:
            i += 1
            val = struct.unpack('<d', data[i:i+8])[0] if i+8<=n else None
            rows.append((path, 'f64', val))
            i += 8
            continue
...
print(f'Rows outside ParameterListe: {len(rows)}')
for p,t,v in rows[:400]:
    print(f'{t:14} {p}\n  -> {v}')
