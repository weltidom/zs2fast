...
while i < n and count < 100:
    ...
    dtype = data[i]
    # Match the Rust path building exactly
    path = '/'.join(name_stack) + '/' + name
    
    if 'EigenschaftenListe' in path:
        print(f"Found path: {path[:120]}")
        count += 1
    ...
    if dtype in [0xAA, 0x00]:
        ...
        if i >= n:
            break
        dd_len = data[i]  # read length
        i += 1 + dd_len  # skip length byte + payload
...
print(f"\n\nTotal paths with 'EigenschaftenListe': {count}")
